...
def load_data():
    try:
        ...
        df = pd.DataFrame({"url": [], "title": [], "score": [], "comments": [
        ], "age": [], "user_interest": [], "user_ranking_time": []})
    return df
...
def scrape(pages):
    ...
    for i in range(pages):
        ...
        for i in range(0, len(item_list_items) - 2, 3):
            ...
            age = second.find(
                "span", attrs={"class": "age"}).get_text().split()[0]
            ...
            scraped_articles.append(
                (result.get('href'), result.get_text(), score, comments, age))
...
def do_prediction(features):
    ...
    print(
        f"Our prediction for {features['title']}: {prediction > 0} {prediction}")
    print(
        f"Of the new predictions made this session {correct_new_predictions} of {new_predictions} were correct {(float(correct_new_predictions) / float(new_predictions)) * 100.0}%")
...
def score_article(data_frame, article):
    ...
    if not find_article(data_frame, url):
        ...
        data_frame.loc[df.shape[0]] = {"url": url, "title": title, "score": score, "comments": comments,
                                       "age": age, "user_interest": interest, "user_ranking_time": int(time.time())}
